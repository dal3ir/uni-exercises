...
const req_deplacer = function (req, res, query) {
	...
	const level2 = fichierGrille.levels[1];
	grille = level2.map;
	powerPallet = level2.powerPallet;
	pacman = level2.pacman;
	pinky = level2.pinky;
	blinky = level2.blinky;
	clyde = level2.clyde;
	counter = level2.counter;
	point = fichierGrille.global.score;


	cy = Math.floor(action / 15);
	cx = action % 15;
	mur = grille[cy][cx] === 1;
	...
	for (let i = 0; i < powerPallet.length; i++) { // on parcourt les powerPallet 
		if (pacman.x === powerPallet[i] % 15 && pacman.y === Math.floor(powerPallet[i] / 15)) { // si pacman mange une powerPallet 
	...
	if (level2.counter > 0) {
		...
		labyrinthe[pacman.y][pacman.x].image = "assets/media/pacman.png";
		level2.counter = level2.counter - 1;
		contenu = JSON.stringify(fichierGrille); // on convertit le contenu du fichier json en string
		fs.writeFileSync("./src/assets/json/grille.json", contenu, "UTF-8"); // on écrit le contenu dans le fichier json
	...
	if (gagne === true) {	// si gagne est égale à true
		...
		marqueurs.recommencer = `<p>Recommencer la partie en ameliorant votre score</p><a href="/req_debuter2?action=classique"><button> Recommencer </button></a>`
	...
	for (let y = 0; y < labyrinthe.length; y++) {
		...
		for (let x = 0; x < labyrinthe[y].length; x++) {
			...
			if (labyrinthe[y][x].cliquable === 1) {
				if (query.autre === "classique") {
					html += `<a href="req_deplacer2?action=${n}&autre=classique">`;
				}
				else if (query.autre === "feu") {
					html += `<a href="req_deplacer2?action=${n}&autre=feu">`;
				}
				else if (query.autre === "froid") {
					html += `<a href="req_deplacer2?action=${n}&autre=froid">`;
				}
				html += image;
				html += `</a>`;
			} else {
